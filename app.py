...
def extract_fields(user_text, booking):
    ...
    if not booking["guests"]:
        if len(text.split()) <= 3 and not re.search(r'\d{4,}', text):
    ...
    if not booking["name"]:
        ...
        if m:
            ...
            filler = {"hello","hi","hey","yes","no","ok","okay","sure","thanks","hmm","yeah","yep","nope","please"}
    ...
    if not booking["request"] and any(k in low for k in ["view","quiet","balcony","high floor","bed","smoking","non-smoking","extra"]):
...
def voice_text():
    try:
        ...
        response = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[{"role":"system","content":f"You are a hotel receptionist. Collected so far: Name:{booking.get('name')} Email:{booking.get('email')} Phone:{booking.get('phone')} Room:{booking.get('room')} Date:{booking.get('date')} Guests:{booking.get('guests')} Request:{booking.get('request')}. Ask ONE missing field at a time in this order: name, email, phone, room, date, guests. If room is missing, ask which room (Deluxe, Suite, or Luxury). Speak briefly. Never repeat filled fields."}] + chat)
        ...
        if asked and not booking.get("request"):
            if any(w in low for w in ["no","nothing","none","skip"]):
